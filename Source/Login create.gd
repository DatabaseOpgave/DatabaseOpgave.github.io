...
func _execInsert():
	var data = [[str($Username.get_text()), $Email.get_text(), $User_id.get_text()]]
	print(data)
	insertToDB('BEGIN; INSERT INTO "Login" ("Username", "Email", "User_id") VALUES'+" ('%s','%s','%s'); COMMIT;", data)
	_on_ButtonSelect_pressed()
	
func _execSelect():
	
	var data = selectFromDB("BEGIN; SELECT * FROM Login; COMMIT;")
	var return_data = ""
...
func _execUpdate():
	var data = [[str($Username.get_text()), $Email.get_text(), $User_id.get_text()]]
	updateToDB("BEGIN; UPDATE Login SET Username = '%s', Email = %s WHERE User_id = %s; COMMIT;", data)
	_on_ButtonSelect_pressed()
